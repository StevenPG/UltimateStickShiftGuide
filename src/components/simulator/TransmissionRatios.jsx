import { vehiclePresets } from '../../data/defaultVehicles'

const gearCountOptions = [5, 6, 7, 8, 9, 10]

export default function TransmissionRatios({
  gearCount,
  gearRatios,
  selectedPreset,
  onGearCountChange,
  onGearRatioChange,
  onPresetChange,
}) {
  const handleRatioChange = (index, value) => {
    const numValue = parseFloat(value) || 0
    onGearRatioChange(index, numValue)
  }

  // Grid layout based on gear count
  const getGridClass = () => {
    if (gearCount <= 4) return 'grid-cols-2'
    if (gearCount <= 6) return 'grid-cols-2 md:grid-cols-3'
    return 'grid-cols-2 md:grid-cols-4'
  }

  return (
    <div className="card">
      <div className="flex items-center gap-2 mb-4">
        <svg className="w-5 h-5 text-orange-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" />
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
        </svg>
        <h2 className="text-lg font-semibold text-gray-900 dark:text-white">
          Transmission Ratios
        </h2>
      </div>

      {/* Vehicle preset dropdown */}
      <div className="mb-4">
        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
          Vehicle Preset
        </label>
        <select
          value={selectedPreset}
          onChange={(e) => onPresetChange(e.target.value)}
          className="input-field"
        >
          {Object.entries(vehiclePresets).map(([key, preset]) => (
            <option key={key} value={key}>
              {preset.name}
            </option>
          ))}
        </select>
      </div>

      {/* Gear count selector */}
      <div className="mb-4">
        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
          Number of Gears
        </label>
        <select
          value={gearCount}
          onChange={(e) => onGearCountChange(parseInt(e.target.value))}
          className="input-field"
        >
          {gearCountOptions.map(count => (
            <option key={count} value={count}>
              {count}-speed
            </option>
          ))}
        </select>
      </div>

      {/* Gear ratio inputs */}
      <div className={`grid ${getGridClass()} gap-3`}>
        {gearRatios.map((ratio, index) => (
          <div key={index}>
            <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">
              {getGearLabel(index + 1)} Gear
            </label>
            <input
              type="number"
              value={ratio}
              onChange={(e) => handleRatioChange(index, e.target.value)}
              step="0.01"
              min="0.1"
              max="15"
              className="input-field text-center font-mono"
            />
          </div>
        ))}
      </div>
    </div>
  )
}

function getGearLabel(gear) {
  const labels = ['1st', '2nd', '3rd', '4th', '5th', '6th', '7th', '8th', '9th', '10th']
  return labels[gear - 1] || `${gear}th`
}
